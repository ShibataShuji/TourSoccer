...
//			g_ObjectData[i].rot = Rot;		// 配置の角度
//			g_ObjectData[i].scl = Scl;		// 大きさの倍率
//			g_ObjectData[i].size = Size;	// 配置の角度
//
//			g_ObjectData[i].act = 0;
//			g_ObjectData[i].arraynum = 6 * i;
//			g_ObjectData[i].DisappearanceCount = disappearance_count;
...
//
//
//			// 引数の座標などからオブジェクトの端っこの座標を求める
//			// Left Rigt Up Down in Flont Behind
//			D3DXVECTOR3 LUF = D3DXVECTOR3(Pos.x,          Pos.y + Size.y, Pos.z - Size.z);
//			D3DXVECTOR3 RUF = D3DXVECTOR3(Pos.x + Size.x, Pos.y + Size.y, Pos.z - Size.z);
//			D3DXVECTOR3 LDF = D3DXVECTOR3(Pos.x,          Pos.y,          Pos.z - Size.z);
//			D3DXVECTOR3 RDF = D3DXVECTOR3(Pos.x + Size.x, Pos.y,          Pos.z - Size.z);
//
//			// 使わないけど一応。立方体の後ろ側の四隅
//			D3DXVECTOR3 LUB = D3DXVECTOR3(Pos.x,		  Pos.y + Size.y, Pos.z);
//			D3DXVECTOR3 RUB = D3DXVECTOR3(Pos.x + Size.x, Pos.y + Size.y, Pos.z);
//			D3DXVECTOR3 LDB = D3DXVECTOR3(Pos.x,		  Pos.y,		  Pos.z);
//			D3DXVECTOR3 RDB = D3DXVECTOR3(Pos.x + Size.x, Pos.y,		  Pos.z);
//
//
//			// 頂点配列の設定
//			g_VertexArray[6 * i + 0] = { D3DXVECTOR3(LUF.x, LUF.y, LUF.z), D3DXVECTOR3(0.0f, 1.0f, 0.0f), D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f), D3DXVECTOR2(U, V) };
